#!/usr/bin/env python3
# kingandassassins.py
# Author: Sébastien Combéfis, Albert Emile
    ...
    def _nextmove(self, state):
        ...
        # Others turns
            ...
            if self._playernb == 0:
                ...
                # Send all villager near the king and the nearest assassin of the king next to him
                    ...
                    if i < APvillage - 2:
                        ...
                        for n in assassins:
                            if findPos(n) in goal:
                                ...
                                if posKing[0]-findPos(n)[0] == 0:
                                    # In the same line of the king
                                    if posKing[1]-findPos(n)[1] > 0:
                                        # The king is on the right of the assassin on the board
                                        actionslistvillage.append(('attack', findPos(n), 'E'))
                                    else:
                                        actionslistvillage.append(('attack', findPos(n), 'W'))
                                if posKing[1]-findPos(n)[1] == 0:
                                    # In the same column of the king
                                    if posKing[0]-findPos(n)[0] > 0:
                                        # The king is under the assassin on the board
                                        actionslistvillage.append(('attack', findPos(n), 'S'))
                                    else:
                                        actionslistvillage.append(('attack', findPos(n), 'N'))
                        i += 2

                    # To improve
                    # ( Kill the knight(s) on the high right corner )
            ...
            # Play on the king side
                ...
                while j < APknight - 1:
                    # Follow the king
                    for n in knightarround():
                        pass
                        # Follow the path of the king
                        #   --> Goto findPos('king')[0 ou 1] +- 1 or Goto kingpath +/- 1

                        # Try to kill all revealed assassins

                # Arrest if it's possible
                if Fetter and villager in kingSpace('healthy'):
                    pass
                    # Arrest villager if it's possible

                posKnights = findPos('knight')  # List of knights' position
                knightarround = knightarround()

                # King's moves
                # The king follow the path until the castle's door
...
# Path : 2BA\AdvancedPython2BA_Q2\"Projet King&Assassin's"
# Path : Documents\ECAM\(ECAM) Informatique\2BA\AdvancedPython2BA_Q2\"Projet King&Assassin's"

# Launch server : python "Test K&A.py" server --verbose
# Launch client : python "Test K&A.py" client --verbose name

# Missing Knight's moves and ameliorations for the others
